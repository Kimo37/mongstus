...
local function moveFatherTo(targetPosition)
	if not gameActive or fatherMoving then return end
	...
	if success and path.Status == Enum.PathStatus.Success then
		...
		local function moveToNextWaypoint()
			if waypointIndex <= #waypoints and gameActive and fatherMoving then
				...
				connection = fatherHumanoid.MoveToFinished:Connect(function(reached)
					...
					if waypointIndex <= #waypoints and gameActive and fatherMoving then
						...
						-- Movement complete
						print("Father reached destination")
						fatherMoving = false
				...
				spawn(function()
					...
					if fatherMoving and waypointIndex == waypointIndex then
						print("Waypoint movement timed out, moving to next")
						waypointIndex = waypointIndex + 1
						if waypointIndex <= #waypoints and gameActive and fatherMoving then
							moveToNextWaypoint()
						else
		...
		print("Pathfinding failed, using simple MoveTo. Error:", errorMessage)
		fatherHumanoid:MoveTo(targetPosition)
		updateFatherPosition:FireAllClients(targetPosition)
		
		local connection
		connection = fatherHumanoid.MoveToFinished:Connect(function(reached)
			print("Father movement finished (fallback). Reached:", reached)
			fatherMoving = false
			if connection then
				connection:Disconnect()
			end
		...
		spawn(function()
			...
			if fatherMoving then
				print("Father movement timed out (fallback)")
				fatherMoving = false
				if connection then
					connection:Disconnect()
				end
...
local function chooseFatherNextMove()
	if not gameActive or fatherMoving then return end
...
spawn(function()
	wait(math.random(5, 10)) -- Initial delay
