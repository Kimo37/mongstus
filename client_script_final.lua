...
-- Waypoint positions (matching server script exactly)
local waypoints = {
	{position = Vector3.new(-124.497, 5.295, -13.348), name = "Living Room"},    -- Living room
	{position = Vector3.new(-162.4, 5.095, -17.444), name = "Kitchen"},         -- Kitchen
	{position = Vector3.new(-103.4, 5.095, -29.444), name = "Hallway"},         -- Hallway
	{position = Vector3.new(-67.9, 5.095, -28.944), name = "Bathroom"},         -- Bathroom
	{position = Vector3.new(-102.9, 5.095, -11.944), name = "Another Room"},    -- Another room
	{position = Vector3.new(-84.9, 5.095, -30.944), name = "Near Your Room"}    -- Near kid's room (danger zone!)
}
...
-- Function to update dad's location display (FIXED WITH ACTUAL WAYPOINTS)
local function updateDadLocation(position, danger)
	...
	-- Check if in danger (near kid's room or explicit danger signal)
	if danger or (closestWaypoint and closestWaypoint.name == "Near Your Room") then
		status = "⚠️ DANGER! ⚠️"
...
local function useComputer()
	...
	if isAtComputer then
		...
		-- Get computer position properly
		...
		if computer:IsA("Model") then
			if computer.PrimaryPart then
				...
				-- Find first part in computer model
				local function findFirstPart(parent)
					for _, child in pairs(parent:GetChildren()) do
						if child:IsA("Part") then
							return child
...
print("Client script loaded with proper location detection!")