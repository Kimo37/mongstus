-- Enhanced Horror Game Server Script (Fixed Room Detection)
...
local ROOM_DETECTION_RADIUS = 8 -- How close father needs to be to trigger "IN YOUR ROOM"
local BED_POSITION = Vector3.new(-74.4, 2.595, -48.944) -- Adjust if bed is different
...
-- Function to check if father is in the room area
local function isFatherInRoom()
	local distanceToRoom = (fatherRootPart.Position - ROOM_CENTER).Magnitude
	return distanceToRoom <= ROOM_DETECTION_RADIUS
end
	...
	if success and path.Status == Enum.PathStatus.Success then
		...
		local function moveToNextWaypoint()
			if waypointIndex <= #pathWaypoints and gameActive and fatherMoving then
				...
				-- Check if father entered room during movement
				spawn(function()
					while fatherMoving and waypointIndex == waypointIndex do
						if isFatherInRoom() and not isCheckingRoom then
							-- Father wandered into room during normal patrol
							print("Father accidentally entered room during patrol!")
							updateFatherPosition:FireAllClients(ROOM_CENTER, true)
						end
						wait(0.5)
					end
...
-- Main game loop - continuous room detection
RunService.Heartbeat:Connect(function()
	if gameActive and not isCheckingRoom then
		-- Check if father is in room during normal movement
		if isFatherInRoom() then
			-- Father is in the room area, update clients
			updateFatherPosition:FireAllClients(ROOM_CENTER, true)
		end
	end
end)
	...
	player.Chatted:Connect(function(message)
		if player.Name == game.CreatorId or player.Name == "YourUsernameHere" then
			...
			elseif message:lower() == "/status" then
				...
				print("Father in room:", isFatherInRoom())
				print("Father position:", fatherRootPart.Position)
				print("Distance to room:", (fatherRootPart.Position - ROOM_CENTER).Magnitude)
			end
		end
	end)
end)
...
print("Enhanced horror game initialized with smart room detection!")